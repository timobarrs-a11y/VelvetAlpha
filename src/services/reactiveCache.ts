...
const CACHE_BYPASS_TIERS = ['starter', 'plus', 'elite', 'trial'];

export function shouldUseReactiveCache(userMessage: string, tier?: string): boolean {
  if (tier && CACHE_BYPASS_TIERS.includes(tier)) {
    return false;
  }
