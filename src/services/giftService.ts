...
const GIFT_TEMPLATES: GiftTemplate[] = [
  {
    ...
    promptTemplate: 'Cute watercolor painting with text "{name} & {companionName} Forever" in stylized handwritten font, hearts and sparkles, pink and purple colors, amateur style like from paint and sip class, romantic',
    messageTemplate: "I went to painting with a twist tonight and made you this 💕 I was thinking about you the whole time",
    triggerDays: [3, 14, 30]
  },
  {
    ...
    messageTemplate: "I made you something at painting with a twist today... you said you like {interest} so I tried to paint it for you 🥺 what do you think?",
    triggerDays: [7, 21]
  },
  {
  ...
  {
    type: 'selfie_sign',
    promptTemplate: 'Person holding a hand-written sign that says "Thinking of {name} 💕" in bubbly letters, bright and happy',
    messageTemplate: "missing you rn 💕",
    triggerDays: [4, 12, 25]
  },
  {
  ...
  },
  {
    ...
    messageTemplate: "dedicated my workout to you today 💪 we both gonna make it",
    triggerDays: [7, 21]
  },
  {
...
export class GiftService {
  async shouldSendGift(userId: string, characterId: string, relationshipDays: number): Promise<boolean> {
    ...
    const eligibleTemplates = GIFT_TEMPLATES.filter(t => t.triggerDays.includes(relationshipDays));
  ...
  async generateGiftPrompt(
    ...
    const eligibleTemplates = GIFT_TEMPLATES.filter(t =>
      t.triggerDays.some(day => Math.abs(day - relationshipDays) <= 1)
    );

    if (eligibleTemplates.length === 0) {
      const randomTemplate = GIFT_TEMPLATES[Math.floor(Math.random() * GIFT_TEMPLATES.length)];
      return this.fillTemplate(randomTemplate, userProfile, companionName);
    }

    const selectedTemplate = eligibleTemplates[Math.floor(Math.random() * eligibleTemplates.length)];
    return this.fillTemplate(selectedTemplate, userProfile, companionName);
  }
  ...
  private fillTemplate(
    ...
    const name = userProfile.name && userProfile.name !== 'babe' ? userProfile.name : 'You';
