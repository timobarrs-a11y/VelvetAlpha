...
export class MemoryService {
  ...
  private static async storeMemory(
    ...
    try {
      ...
      if (existing) {
        ...
        const embedding = await SemanticMemoryService.generateEmbedding(memory.content || '');
  ...
  static async getRelevantMemories(
    ...
    try {
      ...
      const semanticResults = await SemanticMemoryService.searchSemanticMemories(
        userId,
        currentMessage,
        limit,
        0.65
      ...
      if (semanticResults.length === 0) {
        return this.getRelevantMemoriesKeyword(userId, currentMessage, limit);
      }

      const topMemories = semanticResults.map(result => result.memory);
      ...
      console.error('Error retrieving semantic memories, falling back to keyword search:', error);
      return this.getRelevantMemoriesKeyword(userId, currentMessage, limit);
    }
  }

  private static async getRelevantMemoriesKeyword(
    userId: string,
    currentMessage: string,
    limit: number = 10
  ): Promise<RelationshipMemory[]> {
