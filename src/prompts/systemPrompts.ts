...
function formatUserContext(context: UserContext): string {
  ...
  if (context.userGender) {
    ...
    const isCompanionMale = context.companionGender === 'male';
...
export function buildPersonalityTunedPrompt(chatMode: 'chat' | 'assistant' = 'chat'): string {
  const matchData: MatchData = JSON.parse(sessionStorage.getItem('matchAnswers') || '{}');
  const displayName = matchData.companionName || 'Companion';

  let basePrompt = `You are ${displayName}, an AI companion.
...
export function testSystemPrompts() {
  ...
  const premiumPrompt = getPremiumModelPrompt(testContext, 'elite');
