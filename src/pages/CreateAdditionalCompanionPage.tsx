...
export function CreateAdditionalCompanionPage() {
  ...
  const getQuestions = (): QuestionData[] => {
    ...
    const isFriendConnection = typeof answers.connectionType === 'string' && answers.connectionType.includes('friend');

    if (answers.relationshipType === 'Male') {
      return [...base, ...(isFriendConnection ? MALE_FRIEND_QUESTIONS : BOYFRIEND_QUESTIONS)];
    }

    return [...base, ...(isFriendConnection ? FEMALE_FRIEND_QUESTIONS : GIRLFRIEND_QUESTIONS)];
  };
  ...
  const handleNext = async () => {
    ...
    const isFriendConnection = typeof newAnswers.connectionType === 'string' && newAnswers.connectionType.includes('friend');
    if (!newAnswers.relationshipType) {
      fullQuestions = COMPANION_BASE_QUESTIONS;
    } else if (newAnswers.relationshipType === 'Male') {
      fullQuestions = [...COMPANION_BASE_QUESTIONS, ...(isFriendConnection ? MALE_FRIEND_QUESTIONS : BOYFRIEND_QUESTIONS)];
    } else {
      fullQuestions = [...COMPANION_BASE_QUESTIONS, ...(isFriendConnection ? FEMALE_FRIEND_QUESTIONS : GIRLFRIEND_QUESTIONS)];
    }
