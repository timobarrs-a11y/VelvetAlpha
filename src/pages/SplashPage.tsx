...
export function SplashPage() {
  ...
  // "Lite" mode for touch devices (notably iOS Safari). Large `filter: blur()`
  // radii, many infinitely-animating particles, and stacked backdrop-filter
  // layers saturate the iOS compositor — the entrance animations never finish,
  // so opacity-0 content never becomes visible and the page stops responding to
  // taps. On these devices we shrink the blur, drop the particles, skip
  // backdrop-filter, and render content in its final (visible) state up front
  // via `initial={false}` so nothing depends on an animation completing.
  const lite = useMemo(
    () =>
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden" style={{ background: 'linear-gradient(160deg, #060d1f 0%, #09142e 40%, #0a1628 70%, #071020 100%)' }}>
      ...
      {/* Particles + shooting stars — skipped on touch devices to keep the
          iOS compositor free for content rendering and tap handling. */}
