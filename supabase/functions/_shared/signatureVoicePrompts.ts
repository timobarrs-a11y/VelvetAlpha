// GENERATED from src/config/signatureVoices.ts — edge functions can't import from src/.
// To regenerate after a voice changes:
//   npx esbuild src/config/signatureVoices.ts --bundle --format=cjs --outfile=/tmp/sv.cjs
//   node -e "const { SIGNATURE_VOICES } = require('/tmp/sv.cjs'); const out = {}; for (const v of SIGNATURE_VOICES) out[v.id] = { name: v.name, instruction: v.instruction, examples: v.examples || [] }; console.log(JSON.stringify(out, null, 2));"
// Then paste the JSON into VOICE_PROMPTS below, keeping the header + getVoicePrompt function.
